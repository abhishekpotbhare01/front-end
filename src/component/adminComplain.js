...
function AdminComplain() {
  ...
  const getCompList = async () => {
    const res = await axios.get(ROOT_URL+":/getAllComplaint");
    setCompList(res.data);
  };

  const resolveComplaint = async (id) => {
    const complaint = { compId: id };
    await axios.post(ROOT_URL+":/resolveCompStatus", complaint);
  ...
  const rejectComplaint = async (id) => {
    const complaint = { compId: id };
    await axios.post(ROOT_URL+":/rejectCompStatus", complaint);
